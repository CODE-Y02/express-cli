...
**Solution**: Express Forge validates your `.env` on startup. Ensure all required variables listed in `src/config/index.ts` are present in your `.env` file.
...
**Solution**: Ensure you are closing your database connections and server in an `afterAll` hook. Express Forge handles this by default in the generated boilerplate.
