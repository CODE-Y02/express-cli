...
describe('CLI Integration (Smoke Test)', () => {
  ...
  beforeAll(async () => {
    ...
    const baseTempDir = path.join(os.tmpdir(), 'cef-smoke-');
  ...
  it('should scaffold a project with all defaults using --yes flag', async () => {
    ...
    expect(await fs.pathExists(path.join(targetPath, 'package.json'))).toBe(true);
    expect(await fs.pathExists(path.join(targetPath, 'src/app.ts'))).toBe(true);
    expect(await fs.pathExists(path.join(targetPath, 'src/config/env.ts'))).toBe(true);
    expect(await fs.pathExists(path.join(targetPath, 'src/docs/swagger.ts'))).toBe(true);
    expect(await fs.pathExists(path.join(targetPath, '.env.example'))).toBe(true);

    // Verify the TODO app boilerplate exists
    const todoServicePath = path.join(targetPath, 'src/modules/todos/todos.service.ts');
  ...
  it('should build the generated project successfully', async () => {
    ...
    const typeCheck = await execa('npm', ['run', 'check-types'], { cwd: targetPath });
